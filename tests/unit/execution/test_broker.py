...
class DummyMT5:
    ...
    def last_error(self):  # noqa: D401 - interface
        return (1, "error")
...
def test_send_order_dry_run(tmp_path, monkeypatch):
    ...
    class DummyClient(MT5Client):
        ...
        def market_sell(self, symbol: str, volume: float):  # noqa: D401
            return {"price": 1899.0, "retcode": 10009}
...
def test_send_order_ignored(tmp_path, monkeypatch):
    from xau.execution import Broker, MT5Client
    ...
    monkeypatch.delenv("XAU_ENV", raising=False)
    broker = Broker(DummyClient(), journal_dir=tmp_path)
    result = broker.send_order(FLAT)
